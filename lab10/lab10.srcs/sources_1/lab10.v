...
reg  [31:0] fish_clock,geni_clock,genix_clock,geniy_clock ;

wire [9:0]  pos,genix,geniy;
wire        fish_region;
wire        geni_region;
...
// The following code describes an initialized SRAM memory block that
// stores a 320x240 12-bit seabed image, plus two 64x32 fish images.
sramgeni #(.DATA_WIDTH(12), .ADDR_WIDTH(18), .RAM_SIZE(FISH_W*FISH_H*8))
...
always @ (posedge clk) begin
  if (~reset_n)
    ...
    genis_addr <= geni_addr[geni_clock[25:23]] +
